...
# バイクフィッティングで重要な角度の定義
# (点A, 頂点, 点B) の順で指定
ANGLE_DEFINITIONS: dict[str, tuple[int, int, int, str]] = {
...
# キーポイント1点の型: (x_pixel, y_pixel, confidence)
...
def run_inference(
    image_path: str,
    model_path: str = "models/movenet_lightning.onnx",
    ...
    keypoints_raw = outputs[0][0][0]  # shape: (17, 3) -> y, x, confidence

    keypoint_pixels: list[Keypoint] = []
    for i, kp in enumerate(keypoints_raw):
        y, x, conf = kp
        px = int(x * w)
        py = int(y * h)
        keypoint_pixels.append((px, py, float(conf)))
        logger.debug("  %-15s: (%4d, %4d)  conf=%.3f", KEYPOINT_NAMES[i], px, py, conf)
...
def draw_results(
    ...
    for key, (a, b, c, label) in ANGLE_DEFINITIONS.items():
        if all(keypoints[idx][2] > CONF_THRESHOLD for idx in [a, b, c]):
            ...
            logger.info("  %-18s: %6.1f deg", label, angle)
            ...
            logger.debug("  %-18s: LOW CONFIDENCE (skipped)", label)
...
if __name__ == "__main__":
    ...
    logger.info("=== Bike Fitting Analyzer PoC ===")
